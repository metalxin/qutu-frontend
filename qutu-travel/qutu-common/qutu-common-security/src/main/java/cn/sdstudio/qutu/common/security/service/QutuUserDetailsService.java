 ...
 * @date 2025/05/31
 */
public interface QutuUserDetailsService extends UserDetailsService, Ordered {
	...
	default UserDetails getUserDetails(R<UserInfo> result) {
		...
		// 构造security用户
		return new QutuUser(user.getUserId(), user.getDeptId(), user.getUsername(),
	...
	/**
	 * 通过用户实体查询用户详情
	 * @param qutuUser 用户实体对象
	 * @return 用户详情信息
	 */
	default UserDetails loadUserByUser(QutuUser qutuUser) {
		return this.loadUserByUsername(qutuUser.getUsername());
	}
