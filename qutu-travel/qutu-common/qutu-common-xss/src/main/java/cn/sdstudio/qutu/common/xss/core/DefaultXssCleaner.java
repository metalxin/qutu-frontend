...
package cn.sdstudio.qutu.common.xss.core;

import cn.hutool.core.util.CharsetUtil;
import cn.sdstudio.qutu.common.xss.config.QutuXssProperties;

import cn.sdstudio.qutu.common.xss.utils.XssUtil;
...
public class DefaultXssCleaner implements XssCleaner {
	...
	private final QutuXssProperties properties;

	public DefaultXssCleaner(QutuXssProperties properties) {
	 ...
	 * @return 文档输出设置对象
	 */
	private static Document.OutputSettings getOutputSettings(QutuXssProperties properties) {
	...
	public String clean(String bodyHtml, XssType type) {
		...
		QutuXssProperties.Mode mode = properties.getMode();
		if (QutuXssProperties.Mode.escape == mode) {
			// html 转义
			return HtmlUtils.htmlEscape(bodyHtml, CharsetUtil.UTF_8);
		}
		else if (QutuXssProperties.Mode.validate == mode) {
			// 校验
