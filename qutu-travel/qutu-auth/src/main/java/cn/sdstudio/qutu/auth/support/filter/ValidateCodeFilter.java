package cn.sdstudio.qutu.auth.support.filter;
...
import cn.sdstudio.qutu.common.core.exception.ValidateCodeException;
...
public class ValidateCodeFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		// 社交登录模式不需要校验验证码
		if (StrUtil.equals(SecurityConstants.SOCIAL, grantType)) {
			filterChain.doFilter(request, response);
			return;
		}

		// 邮箱验证码模式不需要校验图形验证码（验证码在 Provider 中通过 Redis 校验）
		if (StrUtil.equals("email_code", grantType)) {
			filterChain.doFilter(request, response);
			return;
		}
	...
	private void checkCode() throws ValidateCodeException {
		...
		// https://gitee.com/log4j/qutu/issues/IWA0D
		String mobile = request.get().getParameter("mobile");
		if (StrUtil.isNotBlank(mobile)) {
			randomStr = mobile;
		}

		String email = request.get().getParameter("email");
